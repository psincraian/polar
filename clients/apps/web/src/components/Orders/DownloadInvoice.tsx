...
const DownloadInvoice = ({
  ...
  const onModalSubmit = useCallback(
    async (data: schemas['OrderUpdate']) => {
      ...
      // spinner forever, so poll the order as a fallback until it's ready.
      for (let attempt = 0; attempt < 30; attempt++) {
        ...
        const { data: refreshed } = await api.GET(orderURL, {
          params: { path: { id: order.id } },
        })
        if (refreshed?.is_invoice_generated) {
          completeGeneration()
          return
        }
      }
    ...
    [order.id, api, setError, orderURL, invoiceURL, completeGeneration],
  )
  ...
  }, [eventEmitter, order.id, completeGeneration])
