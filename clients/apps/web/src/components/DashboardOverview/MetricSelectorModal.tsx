...
import { ALL_METRICS, MAX_OVERVIEW_METRICS } from '@/utils/metrics'
...
export const MetricSelectorModalContent = ({
  ...
  // At least one metric, up to the maximum. Allowing fewer than the maximum is
  // what lets a user permanently remove widgets from their overview.
  const isFull = selected.length >= MAX_OVERVIEW_METRICS
  const canSave =
    selected.length >= 1 && selected.length <= MAX_OVERVIEW_METRICS
